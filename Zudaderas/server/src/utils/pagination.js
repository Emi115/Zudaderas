...
export const paginationItems = (req, items) => {
    ...
    return {
        links: {
            ...
            previous:
                first >= step ? `?offset=${prevPage}&limit=${step}` : undefined, // Enlace a la página anterior si es posible
            next:
                first < lastPage
                    ? `?offset=${nextPage}&limit=${step}`
                    : undefined, // Enlace a la próxima página si es posible
            last:
                first < lastPage
                    ? `?offset=${lastPage}&limit=${step}`
                    : undefined, // Enlace a la última página si es posible
        },
